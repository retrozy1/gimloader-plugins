const platformerMax = 0.05167007446288351;
...
api.hotkeys.addConfigurableHotkey(
	...
	() => {
		...
		const mouseWorldVector =
			api.stores.phaser.scene.inputManager.getMouseWorldXY();
		...
		const isPlatformer =
			!!api.stores.phaser.mainCharacter.physics.state.gravity;
		...
		if (rightDistance > leftDistance && rightDistance > verticalDistance) {
			interval = setInterval(() => {
				...
				rb.setTranslation(
					{ x: translation.x - Math.min(max, distance), y: translation.y },
					true
				);
			}, tick);
		} else if (
			leftDistance > rightDistance &&
			leftDistance > verticalDistance
		) {
			interval = setInterval(() => {
				const translation = rb.translation();
			...
			interval = setInterval(() => {
				...
				rb.setTranslation(
					{ x: translation.x, y: translation.y + Math.min(max, distance) },
					true
				);
			}, tick);
...
api.net.onLoad(() => {
	...
	api.patcher.before(inputManager, 'getPhysicsInput', () => {
		...
		if (
			currentInput.angle !== null ||
			currentInput.jump ||
			currentInput._jumpKeyPressed
		) {
			stopInterval();
		}
	});
