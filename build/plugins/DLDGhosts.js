 ...
 * @author retrozy
 * @version 0.1.0
 * @downloadUrl https://raw.githubusercontent.com/retrozy1/gimloader-plugins/main/plugins/DLDGhosts.js
 * @needsPlugin Savestates | https://raw.githubusercontent.com/Gimloader/client-plugins/refs/heads/main/build/plugins/Savestates.js
...
// plugins/DLDGhosts/src/Ghosts.svelte
var root = from_html(`<tr><td><input type="checkbox"/></td><td><select><option>On Teleport</option><option>On Movement</option></select></td><td><input/></td><td><input/></td><td><button title="Delete Ghost"><i class="fas fa-trash"></i></button></td></tr>`);
var root_1 = from_html(`<table><thead><tr><th>Enabled</th><th>Start Mode</th><th>Name</th><th>Skin ID</th><th></th></tr></thead><tbody></tbody></table>`);
var root_2 = from_html(`<!> <div class="flex items-center justify-center"><button class="border-2 rounded-md">Upload Input Recording</button></div>`, 1);
function Ghosts($$anchor, $$props) {
  push($$props, true);
  let ghosts = proxy($$props.initialGhosts);
  const updateGhosts = () => {
    $$props.setGhosts(ghosts);
  };
  function uploadJson() {
  ...
  async function addGhost() {
    try {
      const recording = await uploadJson();
      ghosts.push({
        enabled: true,
        mode: "onTeleport",
        name: "",
      ...
      updateGhosts();
      api.UI.message.success({ content: "Added Ghost" });
    } catch (err) {
      api.UI.modal.error({ content: err.message });
    }
  }
  var fragment = root_2();
  var node = first_child(fragment);
  {
    var consequent = ($$anchor2) => {
      ...
      each(tbody, 21, () => ghosts, (ghost) => ghost.id, ($$anchor3, ghost, $$index) => {
        var tr = root();
        var td = child(tr);
        ...
        option.value = option.__value = "onTeleport";
        var option_1 = sibling(option);
        option_1.value = option_1.__value = "onMovementAfterTeleport";
        reset(select);
        init_select(select);
        reset(td_1);
        ...
        var input_3 = child(td_3);
        remove_input_defaults(input_3);
        reset(td_3);
        var td_4 = sibling(td_3);
        var button = only_child(td_4);
        reset(tr);
        bind_checked(input_1, () => get(ghost).enabled, ($$value) => get(ghost).enabled = $$value);
        delegated("change", select, updateGhosts);
        bind_select_value(select, () => get(ghost).mode, ($$value) => get(ghost).mode = $$value);
        delegated("change", input_2, updateGhosts);
        bind_value(input_2, () => get(ghost).name, ($$value) => get(ghost).name = $$value);
        delegated("change", input_3, updateGhosts);
        bind_value(input_3, () => get(ghost).skinId, ($$value) => get(ghost).skinId = $$value);
        delegated("click", button, () => {
          const index2 = ghosts.indexOf(get(ghost));
          if (index2 !== -1) ghosts.splice(index2, 1);
          updateGhosts();
        });
        append($$anchor3, tr);
      });
    ...
    if_block(node, ($$render) => {
      if (ghosts.length > 0) $$render(consequent);
    });
  }
  var div = sibling(node, 2);
  var button_1 = only_child(div);
  delegated("click", button_1, addGhost);
  append($$anchor, fragment);
  pop();
}
delegate(["change", "click"]);

// plugins/DLDGhosts/src/index.ts
var createPhysics = null;
var applyTickInput = null;
var fileCheck = "this.projectileHitForcesHistory=new Map,this.projectileHitForcesQueue=new Set";
...
function createGhost(id, recording, name, skinId) {
  ...
  addGhost(ghost, id);
  const ghostChar = api.stores.phaser.scene.characterManager.characters.get(id);
...
var settings = api.settings.create([
  {
    ...
    type: "customsection",
    default: [],
    render(container, currentValue, update2) {
      mount(Ghosts, {
        target: container,
        props: {
...
api.net.onLoad(() => {
  ...
  const onMovementCallbacks = [];
  const ghostDestroyCallbacks = [];
  const onStateLoaded = () => {
    ...
    for (const ghost of settings.ghosts) {
      ...
      if (!ghost.enabled || startPos.x !== translation.x || startPos.y !== translation.y)
        continue;
      const startGhost = (recording) => {
      ...
      if (ghost.mode === "onTeleport") {
        ...
        const snippedFrames = movedInInput(recordingFrames[0]) ? recordingFrames : recordingFrames.slice(recordingFrames.findIndex(movedInInput));
        const recording = {
          ...ghost.recording,
        ...
        const onMovement = () => {
          ...
          const index2 = onMovementCallbacks.indexOf(onMovement);
          if (index2 !== -1) onMovementCallbacks.splice(index2, 1);
        };
...
var movedInInput = (input) => input.angle !== null || input.jump || input._jumpKeyPressed;
